...
public class TswitchDAO {
	...
	public List<Tswitch> listPaging(int first, int second, String filter, String orderby) throws Exception {		
    	...
    	oList = session.createSQLQuery("select * from Tswitch "
	...
	public int pageCount(String filter) throws Exception {
		...
		if (filter == null || "".equals(filter))
			filter = "0 = 0";
		session = StoreHibernateUtil.openSession();
		count = Integer.parseInt((String) session.createSQLQuery("select count(*) from Tswitch "
	...
	public List<Tswitch> listNativeByFilter(String filter, String orderby) throws Exception {		
    	List<Tswitch> oList = null;
    	session = StoreHibernateUtil.openSession();
		oList = session.createSQLQuery("select Tswitch.* from Tswitch where " + filter + " order by " + orderby).addEntity(Tswitch.class).list();
		session.close();
        return oList;
    }	
