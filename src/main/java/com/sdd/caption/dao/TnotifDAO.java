...
public class TnotifDAO {
	...
	public List<Tnotif> listPaging(int first, int second, String filter, String orderby) throws Exception {		
    	...
    	oList = session.createSQLQuery("select * from Tnotif "
	...
	public int pageCount(String filter) throws Exception {
		...
		if (filter == null || "".equals(filter))
			filter = "0 = 0";
		session = StoreHibernateUtil.openSession();
		count = Integer.parseInt((String) session.createSQLQuery("select count(*) from Tnotif "
	...
	public List<Vnotif> listNotif(String filter) throws Exception {		
    	...
    	if (filter == null || "".equals(filter))
			filter = "0 = 0";
    	session = StoreHibernateUtil.openSession();
    	oList = session.createSQLQuery("select musergrouppk, mmenupk, sum(notifcount) as totalnotif, notiftxt from Tnotif "
    			+ "join Mmenu on mmenufk = mmenupk join Musergroupmenu on musergroupmenu.mmenufk = mmenu.mmenupk "
				+ "join Musergroup on musergroupfk = musergrouppk where " + filter + " group by musergrouppk, mmenupk, notiftxt")
