...
public class TsecuritiesitemDAO {
	...
	public List<Tsecuritiesitem> listPaging(int first, int second, String filter, String orderby) throws Exception {		
    	...
    	oList = session.createSQLQuery("select * from Tsecuritiesitem "
	...
	public int pageCount(String filter) throws Exception {
		...
		if (filter == null || "".equals(filter))
			filter = "0 = 0";
		session = StoreHibernateUtil.openSession();
		count = Integer.parseInt((String) session.createSQLQuery("select count(*) from Tsecuritiesitem "
	...
	public Tsecuritiesitem findByPk(Integer pk) throws Exception {
		session = StoreHibernateUtil.openSession();
		Tsecuritiesitem oForm = (Tsecuritiesitem) session.createQuery("from Tsecuritiesitem where Tsecuritiesitempk = " + pk).uniqueResult();
		session.close();
		return oForm;
	}
	...
	public Tsecuritiesitem findByFilter(String filter) throws Exception {
		session = StoreHibernateUtil.openSession();
		Tsecuritiesitem oForm = (Tsecuritiesitem) session.createQuery("from Tsecuritiesitem where " + filter).uniqueResult();
		session.close();
		return oForm;
	}
	...
	public String getField(String code) throws Exception {
		session = StoreHibernateUtil.openSession();
		String data = (String) session.createQuery("select description from Tsecuritiesitem where org = '" + code + "'").uniqueResult();
		session.close();
		return data;
	}
	...
	public List<Tsecuritiesitem> listNativeByFilter(String filter, String orderby) throws Exception {
		...
		if (filter == null || "".equals(filter))
			filter = "0 = 0";
		session = StoreHibernateUtil.openSession();
		oList = session.createSQLQuery("select * from Tsecuritiesitem join Tincoming on tincomingfk = tincomingpk where " + filter + " order by " + orderby)
				.addEntity(Tsecuritiesitem.class).list();
		session.close();
		return oList;
	}
