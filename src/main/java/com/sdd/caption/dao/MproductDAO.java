...
public class MproductDAO {
	...
	public List<Mproduct> listPaging(int first, int second, String filter, String orderby) throws Exception {		
    	...
    	oList = session.createSQLQuery("select Mproduct.* from Mproduct join Mproducttype on mproducttypefk = mproducttypepk "
	...
	public int pageCount(String filter) throws Exception {
		...
		if (filter == null || "".equals(filter))
			filter = "0 = 0";
		session = StoreHibernateUtil.openSession();
		count = Integer.parseInt((String) session.createSQLQuery("select count(*) from Mproduct join Mproducttype on mproducttypefk = mproducttypepk "
	...
	public List<Mproduct> listByFilterconcat(String filter, String orderby) throws Exception {		
    	...
    	if (filter == null || "".equals(filter))
			filter = "0 = 0";
    	session = StoreHibernateUtil.openSession();
		oList = session.createSQLQuery("select productgroupcode || ' - ' ||productname as productgroupcode, productgroupcode, productname, qtymin, picname, picemail, productcode, mproductpk, mproductjenisfk  from Mproduct")
				.addEntity(Mproduct.class).list();
		session.close();
        return oList;
    }
	...
	public List getProductOnOrder(String filter) throws Exception {
		List oList = new ArrayList();
       	session = StoreHibernateUtil.openSession();
       	oList = session.createQuery("select distinct productcode from Torderdata where " + filter + " order by productcode").list();   
        session.close();
        return oList;
	}
	
	/*@SuppressWarnings("unchecked")
	public List<Vproductparam> getProductparam(String filter) throws Exception {
		List<Vproductparam> oList = new ArrayList<Vproductparam>();
       	session = StoreHibernateUtil.openSession();
       	oList = session.createSQLQuery("select mproduct.productcode,productname from Torderdata join Mproduct on mproductpk = mproductfk "
       			+ "where " + filter + " group by mproduct.productcode,productname order by mproduct.productcode").addEntity(Vproductparam.class).list();   
        session.close();
        return oList;
	}*/
	...
	public List<Mproduct> startsWith(int maxrow, String value, String param) {
		List<Mproduct> oList = new ArrayList<Mproduct>();
       	session = StoreHibernateUtil.openSession();
       	if (param != null)
       		oList = session.createSQLQuery("select * from Mproduct where " + param + " and productcode like '%" + value + "%' order by productcode limit " + maxrow).addEntity(Mproduct.class).list();       	
       	else oList = session.createSQLQuery("select * from Mproduct where productcode like '%" + value + "%' order by productcode limit " + maxrow).addEntity(Mproduct.class).list();
        session.close();
        return oList;
	}
