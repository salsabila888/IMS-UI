...
public class MorgDAO {
	...
	public List<Morg> listPaging(int first, int second, String filter, String orderby) throws Exception {		
    	...
    	oList = session.createSQLQuery("select * from Morg "
				+ "where " + filter + " order by " + orderby + " limit " + second +" offset " + first)
				.addEntity(Morg.class).list();		
	...
	public int pageCount(String filter) throws Exception {
		...
		if (filter == null || "".equals(filter))
			filter = "0 = 0";
		session = StoreHibernateUtil.openSession();
		count = Integer.parseInt((String) session.createSQLQuery("select count(*) from Morg "
	...
	public String getField(String code) throws Exception {
		session = StoreHibernateUtil.openSession();
		String data = (String) session.createQuery("select description from Morg where org = '" + code + "'").uniqueResult();
		session.close();
		return data;
	}
