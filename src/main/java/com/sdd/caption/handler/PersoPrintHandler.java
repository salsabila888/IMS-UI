...
public class PersoPrintHandler {
	
	public static void doBonPrint(List<Tperso> objList, String outputfile) throws Exception {
		...
		try {
			if (outputfile.equals("pdf")) {
				...
				Sessions.getCurrent().setAttribute("parameters", null);
				Sessions.getCurrent().setAttribute("reportPath", Executions.getCurrent().getDesktop().getWebApp()
						.getRealPath(SysUtils.JASPER_PATH + "/bonkartu.jasper"));
				...
				datamap.put(1, new Object[] { "No", "Tgl Data", "Org", "Tipe Kartu", "Kode Kartu",
						"Jenis Kartu", "Jumlah", "Petugas", "Paraf" });
				no = 2;
				for (Tperso data : objList) {
					datamap.put(no,
							new Object[] { no - 1, dateLocalFormatter.format(data.getOrderdate()),
									mapOrg.get(data.getMproduct().getMproducttype()
											.getProductorg()),
									data.getMproduct().getMproducttype().getProducttype(),
									data.getMproduct().getProductcode(), data.getMproduct().getProductname(), data.getTotaldata(),
