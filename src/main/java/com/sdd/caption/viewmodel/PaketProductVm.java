									...
									for (Entry<Integer, Tperso> entry : mapData.entrySet()) {
										...
										try {
											...
											for (Tpersodata data : tpersodataDao.listByFilter("tperso.tpersopk = "
													+ tperso.getTpersopk() + " and isgetpaket = 'N'", "tpersodatapk")) {
												...
												FlowHandler.doFlow(session, data.getTembossbranch(), null,
														AppUtils.PROSES_PAKET,
											...
											if (menu != null) {
												NotifHandler.doNotif(session, menu,
														"Order paket baru yang harus di DONE status");
											}
