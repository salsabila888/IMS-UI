...
public class MpersovendorVm {
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		path_root = Executions.getCurrent().getDesktop().getWebApp().getRealPath(AppUtils.FILES_ROOT_PATH + AppUtils.IMAGE_PATH);
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mpersovendor>() {
				...
				@Override
				public void render(Listitem item, final Mpersovendor data, int index)
						throws Exception {
			...
			public void onEvent(Event event) throws Exception {
				if (listbox.getSelectedIndex() != -1) {
					...
					if (objForm.getVendorlogo() != null && objForm.getVendorlogo().trim().length() > 0) {
						imgvendor.setSrc(AppUtils.FILES_ROOT_PATH + AppUtils.IMAGE_PATH + "/" + objForm.getVendorlogo());						
					} else imgvendor.setSrc(null);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {												
				String vendorcode = (String) ctx.getProperties("vendorcode")[0]
						.getValue();	
				String vendorname = (String) ctx.getProperties("vendorname")[0]
						.getValue();
					
				if (vendorcode == null || "".equals(vendorcode.trim())) 
					this.addInvalidMessage(ctx, "vendorcode",
							Labels.getLabel("common.validator.empty"));
				if (vendorname == null || "".equals(vendorname.trim()))
					this.addInvalidMessage(ctx, "vendorname",
							Labels.getLabel("common.validator.empty"));		
			}
		};
	}
