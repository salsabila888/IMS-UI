									...
									try {
										if (data.getStatus().equals(AppUtils.STATUS_DERIVATIF_SCAN)) {
											...
											Mmenu menu = new MmenuDAO().findByFilter(
													"menupath = '/view/order/ordertab.zul' and menuparamvalue = 'crop'");
											if (menu != null) {
												NotifHandler.doNotif(session, menu,
														"Order KARTU BERFOTO baru yang harus di CROP");
											}
										} else if (data.getStatus().equals(AppUtils.STATUS_DERIVATIF_CROP)) {
											data.setCropdate(dateverify);
											data.setMemo(memo);
											Mmenu menu = new MmenuDAO().findByFilter(
													"menupath = '/view/order/ordertab.zul' and menuparamvalue = 'merge'");
											if (menu != null) {
												NotifHandler.doNotif(session, menu,
														"Order KARTU BERFOTO baru yang harus di MERGE");
											}
											...
											if (menu != null) {
												NotifHandler.doNotif(session, menu,
														"Order KARTU BERFOTO baru yang harus diOrder Perso");
											}
										}
									...
									for (Entry<Integer, Tderivatif> entry : mapData.entrySet()) {
										...
										try {
											List<Tderivatifproduct> objList = new TderivatifproductDAO().listByFilter(
													"tderivatif.tderivatifpk = " + data.getTderivatifpk(),
													"tderivatifproductpk");
											for (Tderivatifproduct obj : objList) {
												...
												if (menu != null) {
													NotifHandler.doNotif(session, menu,
															"Order KARTU BERFOTO baru yang harus diApproval Produksi");
												}
											}
			...
			try {
				...
				for (Tperso obj : objSelected) {
					...
					if (action.equals(AppUtils.STATUS_APPROVED)) {
						...
						Mmenu menu = new MmenuDAO().findByFilter(
								"menupath = '/view/order/ordertab.zul' and menuparamvalue = 'invoutapproval'");
						if (menu != null) {
							NotifHandler.doNotif(session, menu,
									"Order perso KARTU BERFOTO baru yang harus diApproval Outgoing");
						}
					}
				}
		...
		for (Entry<Integer, Tderivatif> entry : mapData.entrySet()) {
			...
			try {
				List<Tperso> objSelected = tpersoDao.listByFilter(
						"tderivatifproduct.tderivatif.tderivatifpk = " + data.getTderivatifpk(), "tderivatifproductpk");
				for (Tperso obj : objSelected) {
					List<Toutgoing> outList = toutgoingDao.listByFilter("tpersofk = " + obj.getTpersopk(),
							"toutgoingpk");
					...
					if (action.equals(AppUtils.STATUS_APPROVED)) {
						...
						if (menu != null) {
							NotifHandler.doNotif(session, menu,
									"Order perso KARTU BERFOTO baru yang harus di DONE status produksi");
						}
					}
				}
								...
								if (event.getName().equals("onOK")) {
									...
									try {
										...
										for (Entry<Integer, Tderivatif> entry : mapData.entrySet()) {
											...
											transaction = session.beginTransaction();
											try {
												for (Tperso objForm : tpersoDao.listByFilter(
												...
												if (menu != null) {
													NotifHandler.doNotif(session, menu,
															"Order perso KARTU BERFOTO baru yang harus diOrder Paket");
												}
									...
									for (Entry<Integer, Tderivatif> entry : mapData.entrySet()) {
										...
										try {
											for (Tperso tperso : tpersoDao.listByFilter(
												...
												if (menu != null) {
													NotifHandler.doNotif(session, menu,
															"Order paket KARTU BERFOTO baru yang harus di DONE paket");
												}
											}
										...
										try {
											for (Tpaket objPaket : tpaketDao.listByFilter(
												...
												for (Tpaketdata data : listPaketdata) {
													...
													if (menu != null) {
														NotifHandler.doNotif(session, menu,
																"Order paket KARTU BERFOTO baru yang harus diOrder Delivery");
													}
												}
