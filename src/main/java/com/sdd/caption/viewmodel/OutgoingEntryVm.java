...
public class OutgoingEntryVm {
	...
	public void doProductLoad(@BindingParam("item") String item) {
		if (item != null) {
			try {
				mproductmodel = new ListModelList<>(AppData.getMproduct("productgroup = '" + item + "'", "productcode"));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public void doDelete(Toutgoing obj) {
		if (obj.getStatus().equals(AppUtils.STATUS_INVENTORY_OUTGOINGAPPROVED)) {
			...
			try {		
				obj.getMproduct().getMproducttype().setStockreserved(obj.getMproduct().getMproducttype().getStockreserved() - obj.getItemqty());
				oDao.delete(session, obj);			
				transaction.commit();			
				Clients.showNotification(
						"Hapus data outgoing berhasil.",
	...
	public void doSave() {
		...
		if (stockreserved == null)
			stockreserved = 0;
		if (objForm.getMproduct().getMproducttype().getLaststock() != null && objForm.getItemqty() <= objForm.getMproduct().getMproducttype().getLaststock() - stockreserved) {
			session = StoreHibernateUtil.openSession();
			transaction = session.beginTransaction();
			try {						
				...
				/*objForm.getMproduct().getMproducttype().setStockreserved(objForm.getMproduct().getMproducttype().getStockreserved() != null ? objForm.getMproduct().getMproducttype().getStockreserved() + objForm.getItemqty() : objForm.getItemqty());
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				String outgoingid = (String) ctx
						.getProperties("outgoingid")[0].getValue();
				...
				if (outgoingid == null || outgoingid.trim().length() == 0)
					this.addInvalidMessage(ctx, "outgoingid",
