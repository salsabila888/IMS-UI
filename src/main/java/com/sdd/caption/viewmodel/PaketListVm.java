...
public class PaketListVm {
		...
		try {
			if (filter.length() > 0) {
				...
				if (listData != null && listData.size() > 0) {
					...
					Messagebox.show("Data tidak tersedia", WebApps.getCurrent().getAppName(), Messagebox.OK, Messagebox.INFORMATION);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
								...
								if (event.getName().equals("onOK")) {
									...
									for (Entry<Integer, Tpaket> entry : mapData.entrySet()) {
										...
										try {
											...
											totaldone = 0;
											List<Tpaketdata> listPaketdata = tpaketdataDao.listByFilter("tpaket.tpaketpk = " + objPaket.getTpaketpk() + " and isdlv != 'Y'", "tpaketdatapk");
											for (Tpaketdata data: listPaketdata) {
												...
												data.getTembossbranch().setStatus(AppUtils.STATUSBRANCH_PENDINGDELIVERY);
											...
											if (menu != null) {
												NotifHandler.doNotif(session, menu,
														"Paket kartu baru yang harus dibuat manifest delivery dimenu");
											}
									...
									if (error.length() > 0)
										Messagebox.show(error, WebApps.getCurrent().getAppName(),
												Messagebox.OK, Messagebox.ERROR);
									else 
										Messagebox.show("Proses update status done paket selesai", WebApps.getCurrent().getAppName(),
	...
	public void doPrintOutput() {
		if (mapData.size() == 0) {
			...
			try {
				...
				for (Entry<Integer, Tpaket> entry : mapData.entrySet()) {
					Tpaket data = entry.getValue();
					objList.addAll(tpaketdataDao.listByFilter("tpaket.tpaketpk = " + data.getTpaketpk(), "tpaketdatapk"));					
				}
	...
	public void doSearch() {
		if (processtime != null) {
			if (type.equals("O"))
				filter = "orderdate = '" + dateFormatter.format(processtime) + "' and tpaket.productgroup = '" + AppUtils.PRODUCTGROUP_CARD + "'";
			else
				filter = "Date(processtime) = '" + dateFormatter.format(processtime) + "' and tpaket.productgroup = '" + AppUtils.PRODUCTGROUP_CARD + "'";
			...
			else filter += " and tpaket.status != '" + AppUtils.STATUS_DELIVERY_PAKETORDER + "'";
