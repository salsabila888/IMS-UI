...
public class PinmailerUploadVm {
	...
	private Map<String, List<Tpinmailerdata>> mapDataBranchHeader = new HashMap<>();
	...
	public void doSave() {
		if (objForm.getMproduct() != null) {
			if (media != null) {
				...
				if (isValid) {
					...
					try {
									...
									if (linecontent > 7) {
										...
										if (cardno.length() >= 16 && !cardno.contains("*")) {
											if (line.substring(34, 35).equals("Y")) {
												...
												Tpinmailerproduct tpmp = mapTproduct.get(data.getBranchid()
														+ data.getProductcode() + data.getIsinstant());
												if (tpmp == null) {
													...
													if (data.getMproduct() != null) {
														Morg morg = mapOrg.get(
																data.getMproduct().getMproducttype().getProductorg());
														if (morg != null) {
													...
													if (tpmp.getMproduct() == null) {
														...
														if (missingProduct == null) {
															...
															missingProduct
																	.setTotaldata(missingProduct.getTotaldata() + 1);
														}
														mapMissingProduct.put(
																data.getProductcode() + data.getIsinstant(),
																missingProduct);
													}
												...
												mapTproduct.put(data.getBranchid() + data.getProductcode()
														+ data.getIsinstant(), tpmp);
						...
						try {
									...
									if (entry.getValue().getMbranch() == null) {
										...
										for (Tpinmailerdata data : listTdata) {
											...
											if (data.getCardno().length() > 16) {
												...
												System.out.println(
														"KODE CABANGNYA : " + data.getCardno().substring(10, 13));
												if (oBranch != null) {
													data.setTpinmailerbranch(entry.getValue());
													data.setBranchid(oBranch.getBranchid());
													List<Tpinmailerdata> dataList = mapDataBranchHeader
															.get(oBranch.getBranchid());
													if (dataList == null) {
														dataList = new ArrayList<>();
													}
													dataList.add(data);
													mapDataBranchHeader.put(oBranch.getBranchid(), dataList);
							...
							Map<String, Tpinmailerbranch> mapPmInCardBranch = new HashMap<>();
							Map<String, List<Tpinmailerproduct>> mapListPmInCardProduct = new HashMap<>();
							Map<String, List<Tpinmailerdata>> mapListPmInCardData = new HashMap<>();
							for (Entry<String, List<Tpinmailerdata>> entry : mapDataBranchHeader.entrySet()) {
								Map<String, Tpinmailerproduct> mapPmInCardProduct = new HashMap<>();
								List<Tpinmailerproduct> listTpmproduct = new ArrayList<>();
								List<Tpinmailerdata> listTpmdata = new ArrayList<>();
								Tpinmailerbranch objPmBranch = mapTbranch.get(entry.getKey());
								if (objPmBranch != null) {
									objPmBranch.setTotaldata(objPmBranch.getTotaldata() + entry.getValue().size());
								} else {
									objPmBranch = new Tpinmailerbranch();
									objPmBranch.setTpinmailerfile(objForm);
									objPmBranch.setBranchid(entry.getKey());
									objPmBranch.setMbranch(mapBranch.get(entry.getKey()));
									objPmBranch.setOrderdate(entry.getValue().get(0).getOrderdate());
									objPmBranch.setStatus("");
									objPmBranch.setTotaldata(entry.getValue().size());
								}

								List<Tpinmailerdata> objList = entry.getValue();
								for (Tpinmailerdata data : objList) {

									List<Tpinmailerproduct> listTproduct = mapListproduct.get(entry.getKey());
									if (listTproduct != null)
										listTpmproduct.addAll(listTproduct);

									Tpinmailerproduct tpmp = mapPmInCardProduct
											.get(data.getBranchid() + data.getProductcode() + data.getIsinstant());
									if (tpmp == null) {
										tpmp = new Tpinmailerproduct();
										tpmp.setOrderdate(data.getOrderdate());
										tpmp.setEntrytime(new Date());
										...
										listTpmproduct.add(tpmp);
									} else {
										tpmp.setTotaldata(tpmp.getTotaldata() + 1);
									}
									mapPmInCardProduct.put(
											data.getBranchid() + data.getProductcode() + data.getIsinstant(), tpmp);

									data.setTpinmailerproduct(tpmp);
									listTpmdata.add(data);
								}

								mapPmInCardBranch.put(objPmBranch.getBranchid(), objPmBranch);
								mapListPmInCardProduct.put(objPmBranch.getBranchid(), listTpmproduct);
								mapListPmInCardData.put(objPmBranch.getBranchid(), listTpmdata);
							}

							transaction = session.beginTransaction();
							for (Entry<String, Tpinmailerbranch> entry : mapPmInCardBranch.entrySet()) {
								entry.getValue().setOrderdate(date);
								branchDao.save(session, entry.getValue());
								List<Tpinmailerproduct> listTproduct = mapListPmInCardProduct
										.get(entry.getValue().getBranchid());
								for (Tpinmailerproduct product : listTproduct) {
									product.setTpinmailerbranch(entry.getValue());
									product.setTotalproses(0);
								...
								List<Tpinmailerdata> listTdata = mapListPmInCardData
										.get(entry.getValue().getBranchid());
								for (Tpinmailerdata data : listTdata) {
									data.setTpinmailerbranch(entry.getValue());
									dataDao.save(session, data);
				...
				Messagebox.show("Silahkan upload file order pin mailer", WebApps.getCurrent().getAppName(),
						Messagebox.OK, Messagebox.INFORMATION);
			}
		} else {
			product = Labels.getLabel("common.validator.empty");
		}
	}
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									doReset();
									BindUtils.postNotifyChange(null, null, PinmailerUploadVm.this, "objForm");
