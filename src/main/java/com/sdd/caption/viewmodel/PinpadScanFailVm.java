...
public class PinpadScanFailVm {
	...
	private List<Tpinpadorderdata> listData;
	private Torder obj;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view,
			@ExecutionArgParam("listFail") List<Tpinpadorderdata> listFail, @ExecutionArgParam("obj") Torder obj) {
	...
	private void doRenderData() {
		try {
			if (listData != null && !listData.isEmpty()) {
				Rows rows = new Rows();
				int index = 1;
				for(Tpinpadorderdata data : listData) {
					Row row = new Row();
					Label lbl = new Label();
	...
	public void doExport() {
		try {
			if (listData != null && listData.size() > 0) {
				...
				no = 2;
				for (Tpinpadorderdata data : listData) {
					datamap.put(no, new Object[] { no - 1, data.getTid(), data.getMemo() });
					no++;
					total++;
				}
