...
public class CompanydataVm {
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		try {
			...
			List<Msysparam> params = oDao.listByFilter("paramgroup = '" + AppUtils.PARAM_GROUP_COMPANYDATA + "'", "orderno");
			Row row = null;
			for (Msysparam obj: params) {
				row = new Row();
				row.appendChild(new Label(obj.getParamname()));				
				if (obj.getIsmasked().equals("I")) {
					final Image img = new Image(AppUtils.FILES_ROOT_PATH + AppUtils.IMAGE_PATH + "/" + obj.getParamvalue());
					img.setHeight("70px");
					img.setWidth("130px");
