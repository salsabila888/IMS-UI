	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mregion>() {
				...
				@Override
				public void render(Listitem item, final Mregion data, int index)
						throws Exception {
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {			
				String regionid = (String) ctx.getProperties("regionid")[0]
						.getValue();	
				String regioncode = (String) ctx.getProperties("regioncode")[0]
						.getValue();	
				String regionname = (String) ctx.getProperties("regionname")[0]
						.getValue();
					
				if (regionid == null || "".equals(regionid.trim())) 
					this.addInvalidMessage(ctx, "regionid",
							Labels.getLabel("common.validator.empty"));
				if (regioncode == null || "".equals(regioncode.trim())) 
					this.addInvalidMessage(ctx, "regioncode",
							Labels.getLabel("common.validator.empty"));
				if (regionname == null || "".equals(regionname.trim()))
					this.addInvalidMessage(ctx, "regionname",
							Labels.getLabel("common.validator.empty"));		
			}
		};
	}
