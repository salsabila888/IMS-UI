...
public class InquiryDerivatifCardVm {
	...
	List<Tderivatifdata> objList = new ArrayList<>();
		...
		if (grid != null) {
			grid.setRowRenderer(new RowRenderer<Tderivatifdata>() {
				...
				public void render(Row row, final Tderivatifdata data, int index) throws Exception {
					...
					row.getChildren()
							.add(new Label(data.getTderivatifproduct().getMproduct() != null
