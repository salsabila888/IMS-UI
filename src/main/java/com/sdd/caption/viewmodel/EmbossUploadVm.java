...
public class EmbossUploadVm {
	...
	public void doBrowseProduct(@ContextParam(ContextType.BIND_CONTEXT) BindContext ctx) {
		try {
			...
			if (media != null) {
				if (filename.substring(0, 1).equals("E") || filename.substring(0, 1).equals("A")) {
					...
					Messagebox.show("Penamaan file tidak sesuai dengan format nama file emboss", "Info", Messagebox.OK, Messagebox.INFORMATION);
					btnSave.setDisabled(true);
				}
						...
						try {
							...
							if (objEmbossProduct == null) {
								...
								if (embossdata.getMproduct() != null) {
									Morg morg = mapOrg.get(embossdata.getMproduct().getMproducttype().getProductorg());
									if (morg != null) {
							...
							Tembossbranch objEmbossBranch = mapEmbossBranch.get(
									embossdata.getProductcode() + embossdata.getIsinstant() + embossdata.getBranchid());
							...
							if (embossdata.getMproduct() != null && embossdata.getMproduct().getIsmm().equals("Y")) {
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									doReset();
									BindUtils.postNotifyChange(null, null, EmbossUploadVm.this, "objForm");
								} catch (HibernateException e) {
									transaction.rollback();
