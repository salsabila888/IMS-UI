	...
	public void doSave() {
		if (outstanding == 0) {
			...
			try {
				...
				NotifHandler.doNotif(session, menu, "Order PINPAD baru yang harus di SCAN produksi");

//				FlowHandler.doFlow(session, null, obj.getTorder(), AppUtils.PROSES_OUTGOING, obj.getMemo(),
//						oUser.getUserid());
			...
			if (snList.size() > 0) {
				Messagebox.show(
						"Jumlah yang terpenuhi adalah " + snList.size()
...
//			if (snList.size() > 0) {
//				Messagebox.show(
//						"Jumlah yang terpenuhi adalah " + inList.size()
//								+ ", apakah anda yakin ingin melanjutkan proses?",
//						"Confirm Dialog", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener() {
//							asasa
//						}
//			}
//			Messagebox.show("Masih ada data outstanding. Silahkan selesaikan proses scannya", "Info", Messagebox.OK,
//					Messagebox.INFORMATION);
		}
	}
