					...
					a.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
						...
						public void onEvent(Event event) throws Exception {
							if (data.getTperso() != null) {
								...
								Window win = (Window) Executions.createComponents("/view/perso/persodata.zul", null,
										map);
								win.setWidth("90%");
						...
						public void onEvent(Event event) throws Exception {
							...
							if (data.getProductgroup().trim().equals(AppUtils.PRODUCTGROUP_PINPAD)) {
								...
								win = (Window) Executions.createComponents("/view/inventory/outgoingscantoken.zul",
										null, map);
							} else if (data.getProductgroup().trim().equals(AppUtils.PRODUCTGROUP_DOCUMENT)) {
								win = (Window) Executions.createComponents("/view/inventory/outgoingscandocument.zul",
										null, map);
							}
		...
		try {
			if (year != null && month != null) {
				filter = "extract(year from entrytime) = " + year + " and " + "extract(month from entrytime) = " + month
						+ " and toutgoing.productgroup = '" + productgroup + "'";
	...
	public void doExport() {
		try {
			if (objList != null && objList.size() > 0) {
				...
				cell.setCellValue("Daftar Outgoing");
				rownum++;
				...
				datamap.put(1, new Object[] { "No", "Outgoing Id", "Tipe Produk", "Kode Produk", "Jenis Produk",
						"Waktu Order", "Jumlah", "Status", "Requestor", "Pemutus" });
				no = 2;
				for (Toutgoing data : objList) {
					datamap.put(no, new Object[] { no - 1, data.getOutgoingid(),
							data.getMproduct().getMproducttype().getProducttype(), data.getMproduct().getProductcode(),
							data.getMproduct().getProductname(), datetimeLocalFormatter.format(data.getEntrytime()),
							NumberFormat.getInstance().format(data.getItemqty()),
							AppData.getStatusLabel(data.getStatus()), data.getEntryby(), data.getDecisionby() });
					no++;
					total++;
				}
				...
				String filename = "OUTGOING_LIST" + new SimpleDateFormat("yyMMddHHmm").format(new Date()) + ".xlsx";
