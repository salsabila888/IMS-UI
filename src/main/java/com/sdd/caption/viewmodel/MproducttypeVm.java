...
public class MproducttypeVm {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mproducttype>() {
				...
				public void render(Listitem item, final Mproducttype data, int index)
					...
					item.appendChild(cell);
					cell = new Listcell(data.getStockreserved() != null ? NumberFormat.getInstance().format(data.getStockreserved()) : "0");
					item.appendChild(cell);
	...
	public void delete() {
		try {
			Messagebox.show(Labels.getLabel("common.delete.confirm"), "Confirm Dialog", Messagebox.OK | Messagebox.CANCEL,
...
/*				String productorg = (String) ctx.getProperties("productorg")[0]
						.getValue();*/
				String producttype = (String) ctx.getProperties("producttype")[0]
						.getValue();		
				Integer stockmin = (Integer) ctx.getProperties("stockmin")[0]
						.getValue();	
					
				if (morg == null)
					this.addInvalidMessage(ctx, "morg",
				...
				if (producttype == null || "".equals(producttype.trim()))
					this.addInvalidMessage(ctx, "producttype",
							Labels.getLabel("common.validator.empty"));		
				if (stockmin == null)
					this.addInvalidMessage(ctx, "stockmin",
							Labels.getLabel("common.validator.empty"));	
			}
		};
	}
