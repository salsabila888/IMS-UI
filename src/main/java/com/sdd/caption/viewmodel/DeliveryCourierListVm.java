		...
		if (grid != null) {
			grid.setRowRenderer(new RowRenderer<Tdeliverycourier>() {
				...
				public void render(Row row, final Tdeliverycourier data, int index) throws Exception {
					...
					row.getChildren().add(new Label(data.getMcourier() != null ? data.getMcourier().getCouriername() : ""));
					...
					btnPrint.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
						...
						public void onEvent(Event event) throws Exception {
							...
							parameters.put("URGENTIMG",  data.getIsurgent().equals("Y") ? Executions.getCurrent().getDesktop().getWebApp().getRealPath(AppUtils.FILES_ROOT_PATH + AppUtils.IMAGE_PATH + "/urgent_logo.png") : null);
	...
	public void doSearch() {
		if (processtime != null) {
			...
			if (dlvcourierid != null && dlvcourierid.length() > 0)
				filter += " and dlvcourierid like '%" + dlvcourierid.trim().toUpperCase() + "%'";
			if (mcouriervendor != null)
				filter += " and mcouriervendorfk = " + mcouriervendor.getMcouriervendorpk();
			if (isurgent != null && isurgent.length() > 0)
				filter += " and isurgent like '%" + isurgent.trim().toUpperCase() + "%'";
