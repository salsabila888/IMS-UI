	...
	public void doSearch() {
		try {
			if (itemno == null || itemno.trim().length() == 0)
				...
				objList = TtokenitemDao
						.listNativeByFilter(
								"itemno between '" + itemno.trim() + "' and '" + itemnoend.trim()
										+ "' and status = '" + AppUtils.STATUS_SERIALNO_INJECTED + "'",
								"Ttokenitempk");
			...
			btn.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
				...
				public void onEvent(Event event) throws Exception {
					if (txtAwal == null || txtAwal.getValue().trim().length() == 0)
						...
						objList = TtokenitemDao.listNativeByFilter("serialno between '" + txtAwal.getValue().trim()
								+ "' and '" + txtAkhir.getValue().trim() + "' and status = '"
								+ AppUtils.STATUS_SERIALNO_INJECTED + "'", "Ttokenserialpk");
						int inserted = 0;
						for (Ttokenitem data : objList) {
							if (!listData.contains(data.getItemno().trim())) {
								dataList.add(data);
