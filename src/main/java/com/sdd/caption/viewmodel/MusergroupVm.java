	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Musergroup>() {
				...
				@Override
				public void render(Listitem item, final Musergroup data, int index)
						throws Exception {
					...
					btnMenu.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
						...
						public void onEvent(Event event) throws Exception {
							...
							map.put("obj", data);														
							Window win = (Window) Executions.createComponents("/view/admin/usergroupmenu.zul", null, map);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {												
				String usergroupcode = (String) ctx.getProperties("usergroupcode")[0]
						.getValue();	
				String usergroupname = (String) ctx.getProperties("usergroupname")[0]
						.getValue();								
				...
				if (usergroupname == null || "".equals(usergroupname.trim()))
					this.addInvalidMessage(ctx, "usergroupname",
							Labels.getLabel("common.validator.empty"));				
			}
		};
	}
