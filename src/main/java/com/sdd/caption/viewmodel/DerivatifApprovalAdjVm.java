...
public class DerivatifApprovalAdjVm {
		...
		if (grid != null) {
			grid.setRowRenderer(new RowRenderer<Tderivatif>() {
				...
				public void render(Row row, final Tderivatif data, int index) throws Exception {
					...
					row.getChildren().add(new Label(NumberFormat.getInstance().format(data.getTotaladj())));
					...
					try {
						...
						if (action.equals(AppUtils.STATUS_APPROVED)) {
							for (Tderivatif obj : objSelected) {
								obj.setStatus(AppUtils.STATUS_DERIVATIF_VERIFYDATA);
								obj.setTotaldata(obj.getTotaladj());
							...
							transaction.commit();
							Clients.showNotification("Approval perubahan jumlah order cabang berhasil", "info", null,
									"middle_center", 3000);
						} else {
							for (Tderivatif obj : objSelected) {
							...
							transaction.commit();
							Clients.showNotification("Decline perubahan jumlah order cabang berhasil", "info", null,
									"middle_center", 3000);
						}
						doReset();
					} catch (Exception e) {
	...
	public void doSearch() {
		try {
			filter = "status = " + AppUtils.STATUS_DERIVATIF_APPROVEADJ;
