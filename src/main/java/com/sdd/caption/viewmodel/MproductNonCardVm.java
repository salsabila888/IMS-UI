...
public class MproductNonCardVm {
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("arg") String arg) {
		...
		if (productgroupcode.equals(AppUtils.PRODUCTGROUP_SUPPLIES)) {
			...
			/*HtmlNativeComponent f = (HtmlNativeComponent) divForm.getFirstChild().getChildren().get(15);
			f.setDynamicProperty("style", "display:none");*/
			...
			/*HtmlNativeComponent f = (HtmlNativeComponent) divForm.getFirstChild().getChildren().get(15);
			f.setClientAttribute("style", "display:none");*/
		}
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mproduct>() {
				...
				@Override
				public void render(Listitem item, final Mproduct data, int index)
						throws Exception {
					...
					item.appendChild(cell);
					cell = new Listcell(data.getMproducttype().getLaststock() == null ? "0" : NumberFormat.getInstance().format(data.getMproducttype().getLaststock()));
					item.appendChild(cell);
					cell = new Listcell(data.getMproducttype().getStockinjected() == null ? "0" : NumberFormat.getInstance().format(data.getMproducttype().getStockinjected()));
					item.appendChild(cell);
					cell = new Listcell(data.getMproducttype().getStockmin() == null ? "0" : NumberFormat.getInstance().format(data.getMproducttype().getStockmin()));
					item.appendChild(cell);
				}
			});
		}
	...
	public void refreshModel(int activePage) {
		...
		model = new MproductListModel(activePage, SysUtils.PAGESIZE, filter,
				orderby);
		if (needsPageUpdate) {
							...
							if (event.getName().equals("onOK")) {
								try {
									...
									BindUtils.postNotifyChange(null, null,
											MproductNonCardVm.this, "objForm");
								} catch (HibernateException e) {
									transaction.rollback();
									Messagebox.show(
											"Error : " + e.getMessage(),
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				String productcode = (String) ctx.getProperties("productcode")[0]
						.getValue();
				String productname = (String) ctx.getProperties("productname")[0]
						.getValue();

				if (productcode == null || "".equals(productcode.trim()))
					this.addInvalidMessage(ctx, "productcode",
							Labels.getLabel("common.validator.empty"));
				if (productname == null || "".equals(productname.trim()))
					this.addInvalidMessage(ctx, "productname",
							Labels.getLabel("common.validator.empty"));
			}
		};
	}
