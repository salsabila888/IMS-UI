	...
	public void doSearch() {
		...
		try {
			if (itemno == null || itemno.trim().length() == 0)
				...
				objList = TtokenitemDao.listNativeByFilter("itemno between '" + itemno.trim() + "' and '"
						+ itemnoend.trim() + "' and status = '" + AppUtils.STATUS_SERIALNO_ENTRY + "'",
						"Ttokenitempk");
			...
			btn.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
				...
				public void onEvent(Event event) throws Exception {
					if (txtAwal == null || txtAwal.getValue().trim().length() == 0)
						...
						objList = TtokenitemDao.listNativeByFilter("mproducttypefk = " + obj.getMproduct().getMproducttype().getMproducttypepk() + " and itemno between '" + txtAwal.getValue().trim()
								+ "' and '" + txtAkhir.getValue().trim() + "' and status = '"
