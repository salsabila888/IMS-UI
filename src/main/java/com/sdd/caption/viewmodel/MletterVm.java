	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mletter>() {
				...
				@Override
				public void render(Listitem item, final Mletter data, int index)
						throws Exception {
		...
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {		
				String productgroup = (String) ctx.getProperties("productgroup")[0]
						.getValue();					
				String letterprefix = (String) ctx.getProperties("letterprefix")[0]
						.getValue();	
				...
				if (letterprefix == null || letterprefix.trim().length() == 0) 
					this.addInvalidMessage(ctx, "letterprefix",
							Labels.getLabel("common.validator.empty"));				
			}
		};
	}
