					...
					if (obj != null) {
						if (obj.getStatus().equals(AppUtils.STATUS_SERIALNO_OUTINVENTORY)) {
							...
							try {
								...
								obj.getTincoming().getMproducttype().setStockinjected(obj.getTincoming().getMproducttype().getStockinjected() + 1);
								mproducttypeDao.save(session, obj.getTincoming().getMproducttype());
