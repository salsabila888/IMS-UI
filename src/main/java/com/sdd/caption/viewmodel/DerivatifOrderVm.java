		...
		try {
			if (isEdit != null && isEdit.equals("Y")) {
				...
				if (objDupliacate == null) {
					...
					Mmenu menu = new MmenuDAO().findByFilter("menupath = '/view/derivatif/derivatifapproval.zul'");
					if (menu != null) {
						NotifHandler.doNotif(session, menu, "Order KARTU BERFOTO baru yang harus diApproval Produksi");
					}
					transaction.commit();
					session.close();
