...
public class DeliveryLabelVm {
	...
	@AfterCompose
	@NotifyChange("*")
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("obj") Tdeliverycourier tdeliverycourier, 
			@ExecutionArgParam("mapData") Map<Integer, Tdelivery> mapData,  @ExecutionArgParam("productgroup") String productgroup,
			@ExecutionArgParam("islabelsurat") final String islabelsurat)
		...
		grid.setRowRenderer(new RowRenderer<Tdelivery>() {
			...
			public void render(Row row, final Tdelivery data, int index)
				...
				row.getChildren().add(
						new Label(dateLocalFormatter.format(data
								.getProcesstime())));
	...
	public void doPrintLabel() {
			try {
				...
				zkSession.setAttribute("reportPath", Executions.getCurrent().getDesktop().getWebApp().getRealPath(SysUtils.JASPER_PATH + "/labelpaket.jasper"));
