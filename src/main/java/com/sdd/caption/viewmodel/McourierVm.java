...
public class McourierVm {
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mcourier>() {
				...
				@Override
				public void render(Listitem item, final Mcourier data, int index)
						throws Exception {
					...
					btnQr.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
						...
						public void onEvent(Event arg0) throws Exception {
							Filedownload.save(new File(Executions.getCurrent().getDesktop().getWebApp().getRealPath(
									AppUtils.FILES_ROOT_PATH + AppUtils.QR_PATH + data.getCouriercode().trim() + ".pdf")),
									"application/pdf");
						}
					});
					cell = new Listcell();
	...
	public void save() {
		try {			
			...
			String path_root = Executions.getCurrent().getDesktop().getWebApp().getRealPath(AppUtils.FILES_ROOT_PATH + AppUtils.IMAGE_PATH);
			System.out.println(path_root);
			if (isInsert) {
				objForm.setCouriercode(new TcounterengineDAO()
						.generateCounter(AppUtils.CODE_COURIER));
			...
			if (media != null) {
				...
				if (media.isBinary()) {
					...
					BufferedWriter writer = new BufferedWriter(new FileWriter(path_root + "/" + objForm.getCouriercode()));
	...
	private void qrGenerator(Mcourier obj) throws Exception {
		try {
			String output = Executions.getCurrent().getDesktop().getWebApp().getRealPath(
							AppUtils.FILES_ROOT_PATH + AppUtils.QR_PATH + obj.getCouriercode().trim() + ".pdf");
		    ...
		    BarcodeQRCode q = new BarcodeQRCode(obj.getCouriercode(), 100, 100, new HashMap<EncodeHintType, Object>() {{put(EncodeHintType.CHARACTER_SET, "UTF-8");}});		    
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {		
				String couriername = (String) ctx.getProperties("couriername")[0]
						.getValue();	
				String courieremail = (String) ctx.getProperties("courieremail")[0]
						.getValue();	
				...
				if (couriername == null || "".equals(couriername.trim()))
					this.addInvalidMessage(ctx, "couriername",
							Labels.getLabel("common.validator.empty"));		
				if (courieremail != null && courieremail.trim().length() > 0 && !StringUtils.emailValidator(courieremail)) 
					this.addInvalidMessage(ctx, "courieremail",
