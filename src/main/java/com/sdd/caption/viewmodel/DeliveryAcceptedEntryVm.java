...
public class DeliveryAcceptedEntryVm {
	...
	public void doSearch() {
		try {
			if (mbranch != null) {
				filter = "tglterima is null and mbranchfk = " + mbranch.getMbranchpk();
				...
				try {
					...
					for (Entry<Integer, Tdelivery> entry : mapData.entrySet()) {
						...
						for (Tdeliverydata deliverydata : deliverydataList) {
							if(obj.getIsproductphoto().equals("Y")) {
								FlowHandler.doFlow(session, deliverydata.getTpaketdata().getTpaket().getTderivatifproduct().getTembossbranch(), null,
										AppUtils.PROSES_POD,
										deliverydata.getTpaketdata().getTpaket().getTderivatifproduct().getTembossbranch().getTembossfile().getMemo(),
										oUser.getUserid());
							} else if (deliverydata.getTpaketdata().getTembossbranch() != null) {
