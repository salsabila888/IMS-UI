...
public class CardIncomingEntryVm {
	...
	@Wire
	private Combobox cbTypeProduct, cbCabang;
	@Wire
	...
	public void setProductAutocomplete() {
		try {
			...
			cbTypeProduct.setItemRenderer(new ComboitemRenderer<Mproducttype>() {
				@Override
				public void render(Comboitem item, Mproducttype data, int index)
						throws Exception {
	...
	public void doSave() {
		...
		try {						
			objForm.setIncomingid(new TcounterengineDAO()
					.generateYearMonthCounter(AppUtils.CE_INVENTORY_INCOMING));
			...
			Mmenu menu = new MmenuDAO().findByFilter("menupath = '/view/inventory/incomingapproval.zul' and menuparamvalue = '01'");
			if(menu != null) {
				NotifHandler.doNotif(session, menu, "Data KARTU baru yang harus diApproval Incoming");
			}
