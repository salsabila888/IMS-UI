	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mreturnreason>() {
				...
				@Override
				public void render(Listitem item, final Mreturnreason data, int index)
						throws Exception {
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {												
				String returnreason = (String) ctx.getProperties("returnreason")[0]
						.getValue();	
					
				if (returnreason == null || "".equals(returnreason.trim())) 
					this.addInvalidMessage(ctx, "returnreason",
